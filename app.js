...
(function () {
    ...
    function updateStashPanelHeader() {
        ...
        if (isCreativeMode && isPvP && isBlackSetup) {
            ...
            if (title) title.textContent = tt('setup.pvp_black_turn', '♛ Second player, place your pieces');
            if (hint) hint.textContent = tt('setup.pvp_black_hint', 'Place the black pieces on the top squares, then press "Start Game"');
        } else {
            if (title) title.textContent = tt('setup.stash', 'Item Stash');
            if (hint) hint.textContent = tt('setup.stash_hint', 'Drag onto your pieces or click a piece');
        }
    }
    ...
    function renderStashSetup() {
        ...
        if (infiniteItems) {
            ...
            Object.values(ITEMS_DB).filter(i => !i.disabled).forEach((item) => {
    ...
    function showPieceInvTooltip(piece, cell, canEquip, reason) {
        ...
        if (canEquip) {
            _pihtCount.textContent = window.t('inv.slots.free').replace('{used}', usedText);
            _pihtCount.className = 'piht-count can-equip';
        } else if (reason === 'not_allowed') {
            _pihtCount.textContent = window.t('inv.item.not_allowed');
            _pihtCount.className = 'piht-count full';
        } else {
            _pihtCount.textContent = window.t('inv.slots.full').replace('{used}', usedText);
            _pihtCount.className = 'piht-count full';
        }
    ...
    /**
     * merchants_ring: effective shop price after discounts. The discount
     * applies while a ring is IN THE STASH (between rounds all owned items
     * live there, so "in stash" === "owned" at shop time). Rate is read
     * from the catalog so a balance patch changes it in one place.
     */
    function getEffectiveItemCost(item) {
        let cost = item.cost;
        if (runManager.playerItems.some(i => i && i.id === 'merchants_ring')) {
            const ring = getItemById('merchants_ring');
            const rate = (ring && ring.modifiers && ring.modifiers.shopDiscount) || 0;
            cost = Math.floor(cost * (1 - rate));
        }
    ...
    function renderShopScreen() {
        ...
        currentShopItems.forEach((item, index) => {
            ...
            const effCost = getEffectiveItemCost(item);
            const costHtml = effCost < item.cost
                ? `<s style="opacity:0.5">${item.cost}</s> ${effCost} 🪙`
                : `${item.cost} 🪙`;
            el.innerHTML = `
            ...
            el.addEventListener('click', () => {
                ...
                const result = runManager.buyItem(item, effCost);
                if (result === 'full') {
    ...
    function bindEvents() {
        ...
        document.getElementById('btn-friend-mode')?.addEventListener('click', () => {
            ...
            // NEW (Phase 2, Step 1): a stale forfeit timer from a previous
            // friend-mode session must not fire after we've already left it.
        ...
        /* ================================================================
           NEW (Phase 2, Step 1): reconnect banner + forfeit timer.
           ================================================================
           Shared between host and guest — the underlying disconnect/
           reconnect events are symmetric (see multiplayer.js), so one
           small state machine here covers both roles instead of
           duplicating it in both callback blocks below.
           `_reconnectTimer` is closure-scoped to this setup function
           (not a module-level global) since only one friend-mode game
           is ever active at a time in this client. */
        let _reconnectTimer = null;

        function _showReconnectBanner() {
            const el = document.getElementById('reconnect-banner');
            if (!el) return;
            el.textContent = '⚠️ ' + window.t('mp.reconnect.waiting');
            // Inline styling via CSS variables, same pattern used elsewhere
            // in this file for transient states (e.g. the boss-encounter
            // highlight) — avoids adding a one-off class to style.css for
            // a small, JS-driven, friend-mode-only element.
            el.style.cssText = `
        ...
        /** Called when the opponent's `online` flag drops during an
         *  ACTIVE game (not the lobby — createRoom/joinRoom only wire this
         *  callback into gameplay-phase listeners, see multiplayer.js). */
        function handleOpponentDisconnected() {
            if (!isFriendMode) return; // defensive: ignore if mode was already exited
            _showReconnectBanner();
            const myRole = window.Multiplayer.getRole();
            clearTimeout(_reconnectTimer);
            _reconnectTimer = setTimeout(() => {
                window.Multiplayer.claimForfeitWin(myRole);
                _hideReconnectBanner();
                // showGameOverModalDefault() (NOT showGameOverModal() — that
                // one hardcodes "Play Again" to start a NEW ROGUELIKE RUN,
                // wrong for friend mode) already knows how to route "Play
                // Again" back to the friend lobby when isFriendMode is set.
                showGameOverModalDefault({ iWon: true }); // activates the modal itself, on its own 500ms delay
            }, window.Multiplayer.RECONNECT_TIMEOUT_MS);
        }

        /** Called when the opponent's `online` flag comes back true after
         *  a disconnect was already observed. */
        ...
        document.getElementById('btn-create-room')?.addEventListener('click', () => {
            ...
            const code = window.Multiplayer.createRoom({
                onOpponentReady: () => {
                    document.getElementById('room-waiting-text').textContent = window.t('friend.status.joined');
                ...
                // NEW (Phase 2, Step 1): shared handlers, see their definitions above.
        ...
        document.getElementById('btn-join-room-submit')?.addEventListener('click', () => {
            ...
            if (code.length < 5) {
                document.getElementById('friend-lobby-status').textContent = window.t('friend.status.code_invalid');
                return;
            }
            document.getElementById('friend-lobby-status').textContent = window.t('friend.status.connecting');

            window.Multiplayer.joinRoom(code, {
                onOpponentReady: () => {
                    document.getElementById('friend-lobby-status').textContent = window.t('friend.status.connected');
                },
                // NEW (Phase 2, Step 1): shared handlers, see their definitions above.
                onOpponentDisconnected: handleOpponentDisconnected,
                onOpponentReconnected: handleOpponentReconnected,
        ...
        document.getElementById('btn-friend-lobby-back')?.addEventListener('click', () => {
            ...
            if (window.Multiplayer) window.Multiplayer.cleanup();
            // NEW (Phase 2, Step 1): same stale-timer guard as btn-friend-mode above.
            clearTimeout(_reconnectTimer); _reconnectTimer = null;
            _hideReconnectBanner();
        ...
        if (btnSetupBlack) {
            btnSetupBlack.addEventListener('click', () => {
                if (isBlackSetup) {
                    ...
                    if (panelTitle) panelTitle.textContent = window.t('setup.army.white');
        ...
        document.getElementById('btn-start-game').addEventListener('click', () => {
            if (isRaidSetupMode) {
                ...
                const btn = document.getElementById('btn-start-game');
                btn.disabled = true;
                btn.innerHTML = '<span class="btn-icon">⏳</span> <span>' + window.t('friend.waiting_opponent') + '</span>';
    ...
    function startCreativeMode(pvp) {
        ...
        runManager.playerItems = Object.values(ITEMS_DB)
            .filter(i => !i.disabled) // disabled items are pool-invisible everywhere
            .map(item => ({ ...item, modifiers: { ...(item.modifiers || {}) } }));
    ...
    function showCoinFlip(assignedColor) {
        ...
        const colorLabel = assignedColor === 'white' ? window.t('friend.color.white') : window.t('friend.color.black');
    ...
    function startGameFromSetup() {
        ...
        if (isRoguelikeMode) {
            ...
            _placeScavBotPieces();
            document.getElementById('run-gold').textContent = '';
            document.getElementById('run-round').textContent = window.t('raid.hud.pmc');
    ...
    const PIECE_NAMES = {
        king: window.t('piece.king') || 'King',
        queen: window.t('piece.queen') || 'Queen',
        rook: window.t('piece.rook') || 'Rook',
        bishop: window.t('piece.bishop') || 'Bishop',
        knight: window.t('piece.knight') || 'Knight',
        pawn: window.t('piece.pawn') || 'Pawn'
    };
    ...
    function renderPieceInventoryStash(piece, isReadOnly) {
        ...
        const sourceItems = isCatalogMode
            ? Object.values(ITEMS_DB).filter(i => !i.disabled).map((item) => ({ item, idx: item.id }))
    ...
    function executePlayerMove(fromR, fromC, toR, toC, promotionType) {
        ...
        if (result.captured) {
            ...
            if (isRoguelikeMode && result.captured.color === 'black' && result.captured instanceof PieceEntity) {
                ...
                // iron_will: the attacker has a chance to fish a random
                // item out of every kill. Rolled HERE (economy layer), not
                // in the engine: roguelike is single-player, so this RNG
                // never needs multiplayer outcome-syncing.
                const attacker = engine.board[result.move.to.row][result.move.to.col];
                if (attacker && attacker.getStats) {
                    const dropChance = attacker.getStats().itemDropChance || 0;
                    if (dropChance > 0 && Math.random() < dropChance) {
                        const drop = getRandomItems(1)[0];
                        if (drop) runManager.lootItem({ ...drop, modifiers: { ...(drop.modifiers || {}) } });
                    }
                }
        ...
        // FIX: the old code only consumed POSITIVE goldEarned — with the
        // cursed_gold curse now live (per-quiet-move penalty accrues as
        // negative goldEarned) the delta can go either way. Any nonzero
        // delta is applied and the run total is clamped at zero.
        if (isRoguelikeMode && engine.goldEarned) {
            let delta = engine.goldEarned['white'] || 0;
            if (result.captured) delta += 5; // base 5 gold per capture
            if (delta !== 0) {
                runManager.gold = Math.max(0, runManager.gold + delta);
    ...
    /**
     * @param {?{iWon: boolean}} [forfeitInfo] - NEW (Phase 2, Step 1): when
     *        provided, the game ended by opponent forfeit (reconnect
     *        timeout) rather than a chess result — engine.gameResult was
     *        never set, so the normal white/black/draw branch below is
     *        skipped. The button wiring further down (already correctly
     *        friend-mode-aware) is reused as-is.
     */
    function showGameOverModalDefault(forfeitInfo) {
        const icon = document.getElementById('gameover-icon');
        ...
        if (forfeitInfo) {
            // NOTE: only the `iWon: true` path is reachable in Step 1 — the
            // surviving player calls this themselves after the reconnect
            // timeout. The `false` branch is forward-compatible scaffolding
            // for the (explicitly out-of-scope for Step 1, see the plan)
            // case where the disconnected player later reconnects and needs
            // to be told the game already ended without them.
            icon.textContent = forfeitInfo.iWon ? '🏆' : '💀';
            title.textContent = forfeitInfo.iWon
                ? window.t('mp.reconnect.forfeit_title_win')
                : window.t('mp.reconnect.forfeit_title_lose');
            text.textContent = window.t('mp.reconnect.forfeit_text');
        } else if (engine.gameResult === 'white') {
    ...
    function savePresets(presets) {
        try {
            ...
            const msg = (e && e.name === 'QuotaExceededError')
                ? window.t('presets.err.quota')
                : window.t('presets.err.storage');
            alert(msg);
            return false;
        }
    }
    ...
    function onSavePresetClick() {
        if (!pieceInvTarget) {
            alert(window.t('presets.msg.open_piece'));
            return;
        }
        const piece = engine.getPiece(pieceInvTarget.row, pieceInvTarget.col);
        if (!piece || !(piece instanceof PieceEntity)) return;

        const name = (prompt(window.t('presets.prompt.name')) || '').trim();
        ...
        if (existingIndex !== -1) {
            if (!confirm(window.t('presets.confirm.overwrite').replace('{name}', name))) return;
    ...
    function applyPresetToCurrentPiece(id) {
        ...
        if (applyPiecePreset(piece, preset.data)) {
            ...
            alert(window.t('presets.err.apply'));
        }
    }
    ...
    function renamePresetById(id) {
        ...
        const newName = (prompt(window.t('presets.prompt.rename'), preset.name) || '').trim();
        ...
        if (conflict) {
            alert(window.t('presets.err.name_exists').replace('{name}', newName));
            return;
        }
    ...
    function deletePresetById(id) {
        ...
        const preset = presets.find(p => p.id === id);
        if (!preset) return;
        if (!confirm(window.t('presets.confirm.delete').replace('{name}', preset.name))) return;
    ...
    function buildPresetItem(preset, showApply) {
        ...
        if (showApply) {
            ...
            btnApply.title = window.t('presets.btn.apply_title');
    ...
    function renderPresetsList() {
        ...
        if (presets.length === 0) {
            ...
            empty.textContent = window.t('presets.empty');
    ...
    function bindRaidEvents() {
        ...
        document.getElementById('btn-raid')?.addEventListener('click', () => {
            ...
            const startBtn = document.getElementById('btn-start-game');
            if (startBtn) {
                startBtn.innerHTML = '<span class="btn-icon">⚔️</span> <span>' + window.t('raid.btn.fight') + '</span>';
            }
    ...
    function _handleRaidGameOver() {
        ...
        if (playerWon) {
            ...
            if (title) title.textContent = isRaidScav ? window.t('raid.over.scav_dead') : window.t('raid.over.pmc_dead');
            if (text)  text.textContent  = window.t('raid.over.lost_text');
            if (btnAgain) {
                btnAgain.textContent = window.t('raid.btn.again');
